...
<main style="padding-top: 0px; margin-top: 70px; margin-bottom: 200px;">
    ...
    <form action="{{route('qlsv_diemthi.store')}}" method="post">
        ...
        <table style="width: 100%; ">
            ...
            <tbody>
                @foreach($qlsv_sinhvienlophoc as $value)
                <tr>

                    <td>
                        <?= $stt++ ?>
                        <input type="hidden" name="id_sinhvienlophoc[]" value="{{$value->id_sinhvienlophoc}}">
                    </td>
                    <td>{{$value->hovaten}}</td>
                    <td>
                        <input type="number" style="width:50px;" name="diemlythuyet[]" value="{{$value->diemlythuyet}}">
                    </td>
                    <td>
                        <input type="number" style="width:50px;" name="diemthuchanh[]" value="{{$value->diemthuchanh}}">
                    </td>
                </tr>
                @endforeach
            </tbody>
